...
const SignupPage = () => {
  ...
  const handleGoogleLogin = async (credentialResponse: CredentialResponse) => {
    try {
      ...
      setError((err && err.message) || 'Google sign-in failed. Please try again later.');
  ...
  //     router.push('/dashboard');
  //   } catch (err: any) {
  //     setError((err && err.message) || 'GitHub sign-in failed. Please try again later.');
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#18181c] via-[#23232b] to-[#101014] text-gray-100 font-sans p-4">
        <div className="w-full max-w-md bg-white/5 backdrop-blur-lg border border-white/10 shadow-2xl rounded-2xl p-10 flex flex-col items-center transition-all duration-300">
          <div className="mb-8 text-center">
            <h1 className="text-3xl font-extrabold tracking-tight text-white drop-shadow-lg">Create your <span className="text-[#50f0b4]">Gamer World</span> account</h1>
