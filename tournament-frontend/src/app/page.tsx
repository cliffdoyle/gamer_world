...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated) {
      console.log("User is authenticated, redirecting to /dashboard from homepage."); // Optional: for debugging
      router.push('/dashboard');
    }
  }, [isAuthenticated, isLoading, router]); // Dependencies for useEffect

  // Conditionally render a loader or null to prevent homepage content flash during redirect
  if (isLoading || isAuthenticated) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-black via-slate-900 to-black">
        {/* You can add a subtle loading indicator here if you want */}
        {/* e.g., <div className="text-white">Loading...</div> */}
      </div>
    );
  }
  ...
  // Data for your feature cards (no changes here)
  const features = [
    ...
    { id: 'connect', icon: <FaUsers size={32}/>, title: "Connect & Socialize", description: "Build rich player profiles, form clans/crews, utilize an advanced friend system, and engage in interactive forums.", actionText: "Explore Community", onActionClick: () => router.push('/community')},
    { id: 'share', icon: <FaComments size={32}/>, title: "Strategize & Share", description: "Utilize squad builders, access a custom tactics sharing hub, stay updated with FIFA/FC news, and join clip sharing contests.", actionText: "Discover Tactics", onActionClick: () => router.push('/strategies') },
    ...
    { id: 'experience', icon: <FaShieldAlt size={32}/>, title: "Intuitive Experience", description: "Navigate a sleek, modern interface designed by gamers, for gamers. Effortless and enjoyable to use across all devices.", actionText: "Learn More", onActionClick: () => router.push('/about-us')},
    { id: 'progress', icon: <FaChartLine size={32}/>, title: "Progress & Achievements", description: "Earn exclusive badges, conquer unique challenges, and visually track your skill progression with our robust achievement system.", actionText: "View Achievements", onActionClick: () => router.push('/achievements') },
  ];

  // If not loading and not authenticated, render the actual homepage content
  return (
