...
const RoundRobinTable: React.FC<RoundRobinTableProps> = ({ 
  ...
  const standings = participants.map(p => {
    const participantMatches = matches.filter(
      m => m.participant1_id === p.id || m.participant2_id === p.id
    );
    ...
    participantMatches.forEach(match => {
      ...
      if (match.winner_id === p.id) {
        wins++;
      } else if (match.winner_id && match.winner_id !== p.id) {
        losses++;
      } else if (match.winner_id === null && match.status === 'COMPLETED') {
        draws++;
      }
    });
          ...
          <tbody>
            {standings.map((s, index) => (
              <tr key={s.participant.id}>
                <td>{index + 1}</td>
                <td>{s.participant.participant_name}</td>
                <td>{s.wins}</td>
                <td>{s.losses}</td>
                <td>{s.draws}</td>
                <td>{s.points}</td>
              ...
              {matchesByRound[round]?.map(match => (
                ...
                >
                  <span className="participant">
                    {match.participant1_id ? participantsById[match.participant1_id].participant_name : 'TBD'}
                    {match.status === 'COMPLETED' && ` (${match.score_participant1 || 0})`}
                  </span>
                  <span className="vs">vs</span>
                  <span className="participant">
                    {match.participant2_id ? participantsById[match.participant2_id].participant_name : 'TBD'}
                    {match.status === 'COMPLETED' && ` (${match.score_participant2 || 0})`}
                  </span>
                </div>
              ))}
            </div>
